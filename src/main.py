...
class SyncMicRecognizer(object):
    ...
    def recognize(self):
        if self.recognizer_event.is_set():
            ...
            return
        self._status("listening") #change it so the light activates hee - better for 
        # meeseeks box looks
        
        #If google doesnt want anything else said (like the quiz) play the starting clip
        if not self.recognizer.dialog_follow_on: 
            self.player.play_wav("wavs/lookatme.wav")
	    
        self.recognizer.reset()
        self.recorder.add_processor(self.recognizer)
#        self._status('listening')  dont light it here, see above
        # Tell recognizer to run
        self.recognizer_event.set()

    def endpointer_cb(self):
        #if self.recognizer in self.recorder._processors:
    ...
    def _handle_result(self, result):
        playedstart=False #Had an issue with the finish recognition sound playing
        # twice - this prevents it
        if result.transcript:
            #Again check google isnt expecting more voice input
            if not self.recognizer.dialog_follow_on: 
                self.player.play_wav("wavs/caando.wav")
            playedstart=True
        if result.transcript and self.actor.handle(result.transcript):
            #Check finish recognition sound hasnt been played yet
            #also check google isnt expecting something
            if not playedstart and not self.recognizer.dialog_follow_on:
                self.player.play_wav("wavs/caando.wav")
                playedstart=True
            logger.info('handled local command: %s', result.transcript)
            
            self.player.play_wav("wavs/alldone.wav") #All done!
            
        elif result.response_audio:
        #Google has given use audio to playback
            if not playedstart or self.recognizer.dialog_follow_on:
                self.player.play_wav("wavs/dong.wav")
                
            self._play_assistant_response(result.response_audio) 
            
            if not self.recognizer.dialog_follow_on:  
                self.player.play_wav("wavs/alldone.wav")
                
        elif result.transcript:
            logger.warning('%r was not handled', result.transcript)
            
            #keep your requests *burp* simple
            self.player.play_wav("wavs/simple.wav")
